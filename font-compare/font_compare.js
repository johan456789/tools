class FontComparator {
    ...
    bindEvents() {
        ...
        document.getElementById('load-local-fonts-btn').addEventListener('click', () => {
            this.loadLocalFonts();
        });

        // Sample text buttons
        document.getElementById('plaintext-sample-btn').addEventListener('click', () => {
    ...
    async loadHighlighter() {
        try {
            ...
            const module = await import('https://unpkg.com/@speed-highlight/core/dist/index.js');
            this.highlighter = {
    ...
    updateSyntaxHighlightTheme(theme) {
        ...
        const existingSyntaxStyles = document.querySelectorAll('link[data-syntax-theme]');
        ...
        if (theme === 'dark') {
            syntaxThemeLink.href = 'https://unpkg.com/@speed-highlight/core/dist/themes/github-dark.css';
        } else {
            syntaxThemeLink.href = 'https://unpkg.com/@speed-highlight/core/dist/themes/github-light.css';
        }
    ...
    async loadLocalFonts() {
        ...
        try {
            ...
            console.log(`Successfully loaded ${sortedFonts.length} local font families`);
            ...
            this.showError(`Font access failed: ${error.message}. Using fallback fonts instead.`);
    ...
    populateSystemFonts(fonts) {
        const font1Group = this.elements.font1Select.querySelector('optgroup[label="System Fonts"]');
        const font2Group = this.elements.font2Select.querySelector('optgroup[label="System Fonts"]');
...
console.log(\`Fibonacci(10) = \${result}\`);`;
    ...
    updateFontName(fontKey, name) {
        const nameElement = fontKey === 'font1' ? this.elements.font1Name : this.elements.font2Name;
        const titleElement = fontKey === 'font1' ? this.elements.preview1Title : this.elements.preview2Title;
        
        nameElement.textContent = name;
        titleElement.textContent = name === 'No font selected' ? `Font ${fontKey === 'font1' ? '1' : '2'} Preview` : name;
    }
    ...
    async loadFont(file) {
        ...
        if (!validTypes.includes(file.type) && !validExtensions.includes(fileExtension)) {
            throw new Error(`Invalid font file type. Please upload a TTF, OTF, WOFF, or WOFF2 file.`);
        }
    ...
    updatePreviews() {
        const sampleText = this.elements.sampleText.value.trim() || 
                          '(sample text missing)';

        // Update font 1 preview
        if (this.fonts.font1 && this.fonts.font1.loaded) {
            this.updatePreviewElement(this.elements.preview1, sampleText, this.fonts.font1.family, false);
        } else {
            this.updatePreviewElement(this.elements.preview1, 'Upload a font to see the preview', '', true);
        }

        // Update font 2 preview
        if (this.fonts.font2 && this.fonts.font2.loaded) {
            this.updatePreviewElement(this.elements.preview2, sampleText, this.fonts.font2.family, false);
        } else {
            this.updatePreviewElement(this.elements.preview2, 'Upload a font to see the preview', '', true);
        }
    }
